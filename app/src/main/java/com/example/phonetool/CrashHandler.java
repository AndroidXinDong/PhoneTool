...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread pThread, Throwable pThrowable) {
        if (!handleException(pThrowable) && mExceptionHandler != null) {
            //如果用户没有处理则让系统默认的异常处理器来处理
            mExceptionHandler.uncaughtException(pThread, pThrowable);
        } else {
            try {
                Thread.sleep(3000);
     ...
     */
    private boolean handleException(Throwable pThrowable) {
        if (pThrowable == null) {
            return false;
        }
        //使用Toast来显示异常信息
